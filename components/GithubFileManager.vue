<template>
  <b-modal id="github-file-manager">
    ...
    <template #default="{ hide }">
      ...
      <b-modal id="new-folder-name" title="Create Folder" hide-backdrop>
...
export default Vue.extend({
  ...
  methods: {
    ...
    select() {
      (this as any).$bvModal.hide('github-file-manager')
      this.$root.$emit('path-changed', this.curPath)
    },
    ...
    onClose(evt:any, modalId:string) {
      ...
      if (modalId === 'new-folder-name') this.createFolder()
      else this.dirList = []
    }
  ...
  watch: {
    dirList() { console.log('dirList', this.dirList) }
  }
})
